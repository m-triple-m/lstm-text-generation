...
export function setUpUI() {
  ...
  async function refreshLocalModelStatus() {
    ...
    if (modelInfo == null) {
      modelAvailableInfo.innerText = `No locally saved model for "${textGenerator.modelIdentifier()}".`;
      createOrLoadModelButton.textContent = "Create model";
      deleteModelButton.disabled = true;
      enableModelParameterControls();
    } else {
      modelAvailableInfo.innerText = `Saved @ ${modelInfo.dateSaved.toISOString()}`;
  ...
  createOrLoadModelButton.addEventListener("click", async () => {
    ...
    if (await textGenerator.checkStoredModelStatus()) {
      ...
      const lstmLayerSizes = lstmLayersSizesInput.value
        .trim()
        .split(",")
        .map((s) => parseInt(s));
